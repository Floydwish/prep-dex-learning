...
- [通用问题](#通用问题)
  - [Perp DEX 基础概念与定义](#perp-dex-基础概念与定义)
    - [1. 什么是永续合约？](#1-什么是永续合约)
    - [2. Perp DEX 的核心价值主张](#2-perp-dex-的核心价值主张)
    - [3. Perp DEX 与现货 DEX 的区别](#3-perp-dex-与现货-dex-的区别)
    - [4. 资金费率机制的目的](#4-资金费率机制的目的)
    - [5. 资金费率的计算](#5-资金费率的计算)
  - [交易机制与用户体验](#交易机制与用户体验)
  - [经济模型与流动性](#经济模型与流动性)
  - [风险管理与安全性](#风险管理与安全性)
...
| 对比维度 | 永续合约 | 传统期货 |
|----------|----------|----------|
| **到期日** | 无到期日，可无限期持有 | 有固定到期日 |
| **价格锚定** | 通过资金费率锚定现货价格 | 到期时收敛到现货价格 |
| **交割机制** | 无实物交割，现金结算 | 实物交割或现金结算 |
| **持仓成本** | 支付/收取资金费率 | 无额外持仓成本 |
| **杠杆倍数** | 通常较高（10-100倍） | 相对较低（3-10倍） |

### 📝 详细说明

**永续合约**是一种没有到期日的衍生品合约，交易者可以无限期持有头寸。与传统期货的根本差异在于：

1. **无到期日设计**：永续合约没有固定的到期日，交易者可以随时开仓和平仓，无需担心合约到期问题。

2. **资金费率机制**：为了保持永续合约价格与现货价格的锚定，引入了资金费率机制。当永续价格高于现货价格时，多头支付空头资金费率；反之，空头支付多头。

3. **现金结算**：永续合约采用现金结算，不涉及实物交割，降低了交易复杂性。

4. **高杠杆特性**：由于无需实物交割，永续合约通常提供更高的杠杆倍数。
...
Perp DEX 的核心价值在于将永续合约交易去中心化，解决传统中心化衍生品交易的关键痛点：

1. **信任最小化**：用户无需将资金存入中心化机构，避免了交易所跑路、挪用资金等风险。

2. **透明度**：所有交易、清算、资金费率等过程都在区块链上公开可查，提供前所未有的透明度。

3. **全球化**：不受地域限制，任何有互联网连接的用户都可以参与交易。

4. **资产安全**：用户完全控制私钥，资产安全性不依赖于第三方。

5. **抗审查**：去中心化架构使得单个政府或机构难以完全阻止协议运行。
...
Perp DEX 与现货 DEX 在多个维度存在显著差异：

**交易机制差异**：
- Perp DEX 交易的是永续合约，没有实物交割
- 现货 DEX 交易的是实际的代币资产
- Perp DEX 支持做空，现货 DEX 主要支持做多

**流动性差异**：
- Perp DEX 的 LP 需要承担交易者的盈亏风险
...
资金费率机制是永续合约的核心设计，主要目的包括：

1. **价格锚定**：当永续合约价格高于现货价格时，多头支付空头资金费率，鼓励空头开仓，推动价格下降。

2. **供需平衡**：通过资金费率调节市场参与者的行为，当某一方向持仓过多时，通过费率调整来平衡供需。

3. **风险分担**：将市场风险在多头和空头之间进行合理分配，避免风险过度集中在某一方。

4. **市场稳定**：防止永续价格与现货价格过度偏离，维护市场的稳定性和有效性。
...
**计算示例**：
- 如果永续价格比现货价格高0.1%，多头需要支付空头资金费率
- 费率通常每8小时调整一次，确保价格锚定效果
